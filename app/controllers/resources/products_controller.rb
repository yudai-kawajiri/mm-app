...
class Resources::ProductsController < AuthenticatedController
  ...
  def create
    ...
    respond_to_save(@product, success_path: -> { resources_product_path(@company_from_path, @product) })
  end
  ...
  def destroy
    respond_to_destroy(@product, success_path: resources_products_path(@company_from_path))
  end

  def copy
    @product.create_copy(user: current_user)
    redirect_to resources_products_path(@company_from_path),
  ...
  def purge_image
    ...
    respond_to do |format|
      format.html { redirect_to edit_resources_product_path(@company_from_path, @product), notice: t("resources.products.messages.image_deleted") }
