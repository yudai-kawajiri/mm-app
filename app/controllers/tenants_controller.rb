...
class TenantsController < AuthenticatedController
  ...
  def switch
    ...
    Rails.logger.info "DEBUG: TenantsController#switch - params[:current_tenant_id] = #{params[:current_tenant_id].inspect}"
      tenant = Tenant.find_by(id: params[:current_tenant_id])
      if tenant
        session[:current_tenant_id] = tenant.id
        Rails.logger.info "DEBUG: TenantsController#switch - Setting session[:current_tenant_id] = #{tenant.id}"
      ...
      Rails.logger.info "DEBUG: TenantsController#switch - Clearing session (system admin mode)"
      # システム管理モード（全テナント）
